#!/usr/bin/env bash
# Temporary fresh-image path for a private ronin_puppet branch on Talos 18.04.
set -euo pipefail
...
[ -f "$moonshot_dir/oneshot_1804_x11_talos.sh" ] || {
...
(
    cd "$moonshot_dir"
    SKIP_CONVERGE=1 ./oneshot_1804_x11_talos.sh "$host" --confirm
)

ssh -o BatchMode=yes "$ssh_target" true
printf '\nInstalling first-converge prerequisites...\n'
ssh "$ssh_target" 'bash -s' <<'REMOTE_SETUP'
set -euo pipefail
. /etc/os-release
[ "$ID" = ubuntu ] && [ "$VERSION_ID" = 18.04 ] || {
    printf 'Expected Ubuntu 18.04.\n' >&2
    exit 1
}

export DEBIAN_FRONTEND=noninteractive
apt-get update
apt-get install -y ca-certificates curl git python3-yaml sudo ntp
/etc/init.d/ntp stop
printf 'server ntp.build.mozilla.org iburst\n' > /etc/ntp.conf
ntpd -q -g
...
if [ ! -x /opt/puppetlabs/bin/puppet ]; then
    ...
    curl --fail --location --retry 3 \
        'https://apt.voxpupuli.org/openvox8-release-ubuntu18.04.deb' \
...
ssh "$ssh_target" 'sh -c '\''umask 022; cat > /usr/local/bin/yq-er; chmod 0755 /usr/local/bin/yq-er'\''' \
    < "$ronin_puppet_dir/modules/linux_packages/files/yq-er.sh"
ssh "$ssh_target" 'sh -c '\''set -eu; if [ -e /etc/puppet_role ]; then test "$(cat /etc/puppet_role)" = gecko_t_linux_talos; else printf "%s\n" gecko_t_linux_talos > /etc/puppet_role; fi'\'''
