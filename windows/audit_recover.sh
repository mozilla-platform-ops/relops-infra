...
production_workertype=gecko-t-win10-64-ht
...
function get_status() {
  ...
  for i in $(seq $s $e); do
    ip=10.49.40.$i
    ssh -o ConnectTimeout=5  -o StrictHostKeyChecking=no administrator@$ip powershell -executionpolicy bypass -file $remote_audit_script $production_workertype 2>/dev/null
    ...
    then
      ...
      then
        echo ${name} ssh connection failed
				#echo $name | sed -e "s/^$type//" -e "s/$domain//" >> $downnodes
        echo $ip >> $downnodes
      elif [ $result == 98 ];
      then
      ...
      then
        ...
        #echo $name | sed -e "s/^$type//" -e "s/$domain//" >> $gwdown
         echo $ip >> $downnodes
      fi
    fi
    done
...
function reset_cart () {
  ...
  for c in $(seq 1  7); do
    ...
    then
      ...
      #cat ${c}-${cart_list}
      #readarray -t  c_list  <  ${c}-${cart_list}
      #echo ${c_list[@]} 
    fi
  done
}
