...
Function Get-TreeHerderAlphaResults {
    [CmdletBinding()]
    param (
        [Parameter()]
        [String]
    ...
    if ($Branch -eq "Try") {
        $push_id = (Invoke-RestMethod "https://treeherder.mozilla.org/api/project/try/push/?full=true&count=10&revision=$revision").results.id
    }
    else {
        $push_id = (Invoke-RestMethod "https://treeherder.mozilla.org/api/project/mozilla-central/push/?full=true&count=10&revision=$revision" -TimeoutSec 5).results.id
    }


    if ($Failure) {
        ## Get the treeherder results
    ...
    Write-host "Processing results from tc"
...
function Get-DecisionTaskJson {
    ...
    $Revision | ForEach-Object {
        ...
        Write-host "Creating lookup table"
        ...
        $tc_results = for ($i = 0; $i -lt $tryjob.count; $i++) {
            if (-Not ([string]::IsNullOrEmpty($tryjob.results[$i]))) {
                $collection = $tryjob.results[$i]
                [PSCustomObject]@{
                    "failure_classification_id" = $collection[0]
                    "id"                        = $collection[1]
        ...
        ## foreach task, get the routes
        foreach ($task in $tc_results) {
            if ($task.job_type_name -eq "Gecko Decision Task") {
                $task_def = taskcluster task def $task.task_id | ConvertFrom-Json
                [PSCustomObject]@{
                    Task           = $task
